...
int main(int args, char* argv[]) {
	...
	auto sign_per = [&encrypted](char* plainText, size_t msg_size) -> int
	{
		int encrypted_length =
			priv_sign_sha256(reinterpret_cast<char*>(plainText), msg_size, reinterpret_cast<uint8_t *>(privateKey),
	...
	auto dec_per = [&encrypted, &decrypted](char* plainText, size_t msg_size, size_t encrypted_length) -> bool
	{
		...
		auto computed_hash = get_sha256(reinterpret_cast<char*>(plainText), msg_size);
	...
	bool res = true;
	for (auto i = 0ULL; i < reps; i++) {
		res &= dec_per(reinterpret_cast<char*>(plainText), msg_size, encrypted_length);
	}
	...
	std::cout << "elapsed time: " << elapsed_seconds1.count() << "s for " << reps << " rsa+sha256 (sign)"<< std::endl;
	std::cout << "elapsed time: " << elapsed_seconds2.count() << "s for " << reps << " rsa+sha256 (verify)" << std::endl;
