...
width1 = 0.25
width = 0.50
x2 = [r+width/2 for r in x]
x3 = [r-width/4 for r in x2]
ax.bar(x, lat_64, width1, color= palette[2], hatch="o ", edgecolor="black", label="64B")
ax.bar(x2, lat_128, width1, color= palette[4], hatch="X", edgecolor="black", label="128B")


 #   plt.text(rect.get_x() + rect.get_width() / 2.0, height, f'{height:.0f}', ha='center', va='bottom')
for x, y, p in zip(x, lat_64, lat_64):
    #plt.text(x, y, p, f{p:.0f}, ha='center', va='bottom')
    plt.text(x, y, f'{p:.0f}', ha='center', va='bottom', fontsize=12, weight="bold")
