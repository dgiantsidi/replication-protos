...
DEFINE_uint64(reqs_num, 40e6, "Number of reqs");
...
enum chain_replication { head = 0, middle = 1, tail };
...
void middle_func(app_context *ctx, const std::string &uri1,
	...
	create_session(uri1, 0 /* node 1 */, ctx);
	create_session(uri2, 2 /* node 1 */, ctx);
	ctx->rpc->run_event_loop(100);
	for (auto i = 0ULL; i < FLAGS_reqs_num; i++) {
		// send_req(i, 2, ctx);
		poll(ctx);
...
void forward_req(int dest_node, std::unique_ptr<uint8_t[]> buff, size_t buff_sz, app_context* ctx) {
		// needs to send
		rpc_buffs *buffs = new rpc_buffs();
		...
		// enqueue_req
		ctx->rpc->enqueue_request(ctx->connections[dest_node], kReqForward, &buffs->req,
				&buffs->resp, cont_func,
				reinterpret_cast<void *>(buffs));
}

void send_commit_req(int dest_node, std::unique_ptr<uint8_t[]> buff, size_t buff_sz, app_context* ctx) {
		// needs to send
		rpc_buffs *buffs = new rpc_buffs();
		...
		::memcpy(buffs->req.buf, msg_buf.get(),
				sizeof(cmt_msg));
		// enqueue_req
		ctx->rpc->enqueue_request(ctx->connections[dest_node], kReqCommit, &buffs->req,
				&buffs->resp, cont_func,
				reinterpret_cast<void *>(buffs));
...
/* @dimitra: if you enable this you break the next line due to std::move() */
	...
	if (ctx->node_id == chain_replication::middle)
		forward_req(chain_replication::tail, std::move(batched_msg), req_handle->get_req_msgbuf()->get_data_size(), ctx);
	if (ctx->node_id == chain_replication::tail) 
		send_commit_req(chain_replication::head, std::move(batched_msg), req_handle->get_req_msgbuf()->get_data_size(), ctx);
}
...
int main(int args, char *argv[]) {
	...
	nexus.register_req_func(kReqForward, req_handler);
